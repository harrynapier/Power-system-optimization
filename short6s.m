    ...
    function [fval,fc,fmon,fcar,fdep] = short6s(Ad,Cs,pdis,pcha,batcap,eff,pgrid,Cb,depr,start,batmin,T,outputcarbon,D,weight,avp,x0,A,B,Aeq,Beq,options,coop,trials,A2,A3eq,avda,avga,b1,b2,b3,b4,b5,b6)  
...
problem = createOptimProblem('fmincon','objective',@(x)trafunc6varmoo(x,D,outputcarbon,T,Cb,depr,batcap,weight,eff,b1,b2,b3,b4,b5,b6),'x0',x0,'Aineq',A,'bineq',B,'Aeq',Aeq,'beq',Beq,'lb',LB,'ub',UB,'nonlcon',@(x)tranolin6(x,T,A2,A3eq,B3eq,batmin,eff),'options',options);
...
disp(exitflag);
fval=fg;
[fvv,fmong,fcarg,fdepg]=traf6(xg,D,outputcarbon,T,Cb,depr,batcap,weight,eff,b1,b2,b3,b4,b5,b6);
...
de6 = sum(costs6);
    ...
    fmon = sum(D.*(Pbuy1-Psellback1))+sum(D.*(Pbuy2-Psellback2))+sum(D.*(Pbuy3-Psellback3))+sum(D.*(Pbuy4-Psellback4))+sum(D.*(Pbuy5-Psellback5))+sum(D.*(Pbuy6-Psellback6));
