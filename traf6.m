...
function [f,fmon,fcar,fdep] = traf6(x,D,outputcarbon,T,Cb,depr,batcap,weight,eff,b1,b2,b3,b4,b5,b6)
    mo=3*T;
    Pgrid1 = x(2*T+1:3*T-1); % grid power
   
    Psellback1 = zeros(T,1);
    Pbuy1 = zeros(T,1);
    Ebat1 = x(T+1:2*T-1); % battery SOC
    Pbat1 = x(1:T-1); % battery power
    DeprCo1 = zeros(T,1);
    ...
    [c1,hist,edges,rmm,idx] = rainflow(Ebat1); % rainflow alogrithm on SOC

Ta1 = array2table(c1,'VariableNames',{'Count','Range','Mean','Start','End'}); %convert to array

cyc1 = Ta1{:,1};
dod1 = ((Ta1{:,2}/batcap)).*cyc1; % finding depth of discharge
Cl1=[];
for k = 1 : length(dod1)
    Cl1(k) = 2731.7 * dod1(k)^(-0.679)*exp(1.614*(1-dod1(k))); % degradation equation
end
costs1 = (1./Cl1)*Cb*b1; % degradation cost
de1 = sum(costs1);
...
de6 = sum(costs6);
    ...
    fmon = sum(D.*(Pbuy1-Psellback1))+sum(D.*(Pbuy2-Psellback2))+sum(D.*(Pbuy3-Psellback3))+sum(D.*(Pbuy4-Psellback4))+sum(D.*(Pbuy5-Psellback5))+sum(D.*(Pbuy6-Psellback6)); % electricity cost
    fcar = sum(outputcarbon.*(Pbuy1+Pbuy2+Pbuy3+Pbuy4+Pbuy5+Pbuy6)); % carbon cost
    fdep = de1+de2+de3+de4+de5+de6; % depreciation cost
    f= fmon+fdep + fcar*weight; % total cost with wieghts
